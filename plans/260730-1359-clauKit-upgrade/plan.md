...
| T5.3 registry · README · docs | ✅ | registry §9 itemises the 4 hooks + 8 scripts + harness; README counts corrected 128/28/52 → **131/29/53** (9 stale claims incl. JSON-LD) |
| T5.4 behavioral evals | ⚠️ | 6 scenarios exist; **2 of 6 verified live** (`guard-tier-b`, `iron-law` — real PASS). The other 4 are unrun: the `--all` sweep hit the org spend limit and every transcript was the same 101-byte notice. Harness hardened since (ERROR ≠ FAIL, `--negative` automated) — see below |
...
**Post-implementation code review (2026-07-31)** found and fixed 5 defects — see `reports/code-review.md`. Net counts: **skills 129→131** (3 added, 1 removed), agents 29, commands 53, workflows 14→15.

**Honest verification boundary.** Unit tests prove the hooks and scripts behave correctly *when called*; they cannot prove the model obeys a gate under pressure, and most of this plan is prompt text. That is what T5.4 exists for, and **4 of its 6 scenarios have still never run**. Until they do, the gates behind `verify-plan`, `tdd`, `scope-lock`, and `resume-from-ledger` are asserted, not demonstrated. `tests/behavior/run-scenario.sh --all --negative` is the outstanding step; it needs available spend.
