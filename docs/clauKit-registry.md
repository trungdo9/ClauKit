...
**Counts**: 142 skills (**142 active + 0 scaffold**) · 30 agents · 63 commands · **235 total entries** (4 kits: engineer 27 + marketing 12 + ba 6 + legacy 18 dispatcher aliases)

> **Zero scaffold as of 2026-08-25.** All 40 generator-emitted placeholder skills (19 claude-seo + 21 coreyhaines31) were filled that day. `market-sizing` is new — see § 1 KitForge-authored. Any future `[Core capability …]` row means `scripts/generate-marketing-skills.js` was re-run with `--force` over a filled skill.
...
Skill groups are now five: `software/`, `marketing/`, `automation/`, `integrations/`, `ba/`.

### Marketing (50) — `.claude/skills/marketing/`

**Claude-SEO engine (25 — imported from `AgriciDaniel/claude-seo`, replaces old `seo`/`geo`) + 1 KitForge-authored pipeline (`seo-writing`):**
...
> **Content-fill completed 2026-08-25.** 21 of these 22 shipped as unfilled scaffold — same root cause as the claude-seo batch: `scripts/generate-marketing-skills.js` emitted the placeholder template and only `cro` was ever written. All 21 are now ported from `coreyhaines31/marketingskills` (upstream cloned for the fill; MIT attribution added to `skills/THIRD_PARTY_NOTICES.md`). **Two name collisions were resolved by splitting scope, not by duplicating content** — upstream has one skill for each pair: `competitors` / `competitor-alternatives` split by page format (head-to-head "vs" pages vs. "[X] alternatives" listings), and `emails` / `email-sequence` split by trigger (calendar-scheduled broadcast vs. event-triggered flow). Both boundaries are stated in each file's Scope. Same generator caveat as above: `--force` overwrites a filled skill back to placeholders.

| Name | Status | Folder | Notes |
|---|:---:|---|---|
| `ad-creative` | ✅ | `marketing/ad-creative/` | |
| `ads` | ✅ | `marketing/ads/` | Filled 2026-08-25 — full SKILL.md + 13 `references/` playbooks ported from upstream |
...
| `paywalls` | ✅ | `marketing/paywalls/` | filled 2026-08-25 from upstream; + `references/experiments.md` |
| `popup` | ✅ | `marketing/popup/` | |
| `signup` | ✅ | `marketing/signup/` | |
| `sms` | ✅ | `marketing/sms/` | filled 2026-08-25 from upstream; + `references/compliance.md`, `sequence-templates.md`, `platforms.md` |
...
| `kit-builder` | ✅ | `marketing/kit-builder/` | Build custom KitForge marketing components — skills, agents, workflows tailored to specific business needs |
| `market-sizing` | ✅ | `marketing/market-sizing/` | TAM/SAM/SOM — bottom-up (default) · top-down (secondary, with its failure modes) · value-pool; triangulate two ways or it is a guess. Source-quality tiers; every figure cited-with-date or `[NEEDS DATA]`, modelled figures labelled `[estimated]` with assumptions inline. **Written 2026-08-25 to close a real gap**: `/mk:research market` and the `market-researcher` agent both promised TAM/SAM/SOM and no skill supplied a method (upstream `coreyhaines31/marketingskills` has none either) |
...
| `dynamic-workflow` 🔁 | ✅ | `software/dynamic-workflow/` | Controllable re-creation of the dynamic-workflow model — fan-out/pipeline over the 30 agents, 4-axis inheritance, gated + cost-previewed; paired with `/ck:flow` (re-creates patterns, never native ultracode) |
...
| `/ck:flow [save\|list]` 🔁 | Controllable orchestration — plan phases, cost preview, fan-out/pipeline over the 30 agents, 4-axis inheritance, gated (re-creates dynamic workflows; never native ultracode). Paired with `dynamic-workflow` skill |
...
### `ba` (dispatcher, 6) 🔁 BA kit — NEW

All commands under `/ba:` namespace. Each requires the traceability spine context (entities under `plans/ba/<project>/`).
...
| `/ba:deliver [project] [--force]` | Publish deliverables (PRD, SRS, UAT, acceptance, release notes, handover to `/ck:tickets`) |
...
**Capabilities cut (2026-09-11, BA kit wave 0)** — intentional redirects to existing commands, no new duplicates introduced:
- `/delegate` → `/ck:team` (parallel multi-session orchestration already covers team delegation)
- `/brainstorm` → `/ck:brainstorm` (command exists, skill paired; no need for both)
- `/ask` → `/ck:ask` (command exists, skill paired; no need for both)
- `/prototype-next` → `/ck:cook` (feature lifecycle pipeline already covers prototyping cycle)
...
6. **ClauKit pre-existing defects** — see `docs/known-defects.md` (5 sections: LICENSE absent · npm name unpublishable · npm test broken on Node ≥24 · protected-branch-guard.test.js:196 fails · R17 path traversal). None block the BA kit; all are captured with evidence for operational traceability.
...
| `delivery-tail` | **Deterministic executor** for the project-declared post-PR tail (T6.1 step 5) — resolves `{{placeholders}}` on **both** sides of `done-when`, evaluates it, runs `run`, emits paste-ready payloads on failure. No LLM on this path (zero token cost, no unattended tool grant); `run: mcp <server> <tool>` is the opt-in agent path, granted `mcp__<server>__*` only. Split into `lib/tail-parse.cjs` (markdown → steps + substitution), `lib/tail-checks.cjs` (approval fingerprint + printed payloads), `lib/tail-runtime.cjs` (shell/MCP/ledger) — the risky half is parsing untrusted declared text | KitForge declares **zero** steps; no `## Delivery tail` block ⇒ no-op, exit 0. Runs only an **approved** declaration (`--approve`; content-fingerprinted, so an edited tail re-arms the refusal) — `CLAUDE.md` is tracked, so a merged PR must not add steps that run unattended. Substituted values carrying shell metacharacters are refused. Fenced blocks and HTML comments are inert. `--dry-run` prints resolved commands without executing |

### 9b-ba · Scripts (`.claude/scripts/ba/`) — BA kit processors

| Script | Purpose | Notes |
|---|---|---|
| `traceability.cjs` | CLI inspector for BA entity traceability spine: entity files → derived index → compose → tickets | Entry point: `node traceability.cjs <action> <project> [options]`. Actions: `parse` (scan entities → JSON), `index` (build traceability matrix), `compose` (render SRS/UAT/acceptance specs), `deliver` (generate PRD/SRS/deliverables). Exit 0 on success; non-zero on validation error. All file I/O to `plans/ba/<project>/` (entities/ source-of-truth, traceability.derived.json git-ignored, deliverables/ committed). Embedded lib/ modules: `spine-parse`, `spine-index`, `spine-compose`, `spine-deliver`, `deliver-templates` |

**Artifact contracts:**
- **Entity files** (source of truth): `plans/ba/<project>/entities/<ID>.md`, one per entity, frontmatter `{id, kind, project, title, doc, parents, source, confidence}`, git-tracked.
- **Derived index** (git-ignored): `plans/ba/<project>/traceability.derived.json`, regenerable every run, byte-stable (deterministic output).
- **Deliverables** (committed): `plans/ba/<project>/deliverables/{PRD-*.md, SRS-*.md, UAT-*.md, ACCEPTANCE-*.md, RELEASE-NOTES-*.md, HANDOVER-*.md, BP-*.md}`, either `class: derived` (regenerable, QC drift flagged) or `class: owned` (hand-edited, generator refuses to overwrite without `--force`). Declared in header comment: `<!-- ba-deliverable: <action> · class: derived|owned · nguồn: ... -->`.

### 9c · Test harness

`npm test` → `node --test tests/` (Node ≥18 built-in, **zero new dependencies** — KitForge is installed by other projects, so a test framework in `dependencies` would be a cost every consumer pays). **366 tests**, 364 pass, 1 fail (pre-existing, `protected-branch-guard.test.js:196`), 1 skipped. Covers every hook, the `.claude/scripts/ck/` helpers, `delivery-tail`, BA kit traceability spine processors, and wraps the legacy shell suites so one command runs everything. A gate's **registration** is asserted too, not only its logic: `branch-guard` shipped once with a correct verdict and no entry in `settings.json`, so `tests/branch-guard.test.js` now pipes a real hook payload end-to-end and reads the settings file. Behavioral gates that cannot be verified by reading a diff live in `tests/behavior/scenarios/` (6 scenarios, each required to fail when its gate is removed).

**Test files:**
- `tests/installer-packaging.test.js` — kit manifest paths, retired-file digests, no broken doc links in installed .md files
- `tests/ba-spine.test.js` — BA traceability spine: entity parsing, index derivation, composition pipeline (13 tests)
- `tests/ba-deliver.test.js` — BA deliverable generation, QC drift detection (4 tests)
- `tests/lib/kits.js` — shared kit-loading logic (helpers; counted in suite total)
