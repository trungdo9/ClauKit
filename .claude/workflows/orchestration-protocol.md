...
- **A dispatch has a fixed price before it does any work.** Every subagent except `Explore` re-loads `CLAUDE.md` and the auto-memory index on every request — ~18.6k tokens on one measured workspace, re-read on each of its turns. `Explore` skips both: its first request was ~18k tokens against ~55k for the other types. An agent with `omitClaudeMd: true` loads neither (probe: 20.6k → 2.0k) — but only with a rules card, see § *Subagents load CLAUDE.md* below.
- **Brief discipline.** Narrow brief — one question or one change, files by path, the applicable rules, the exact deliverable. The subagent **returns early** — it stops once it has the answer. **No orchestrator (and no fan-out) for a single lookup** — a one-fact or file-location question is a direct Grep/Read, or at most one `Explore`; every dispatch re-loads a full context before it does any work. Ask for a **result ≤ ~60 lines**; longer detail goes to `plans/<plan>/reports/` and comes back as a path.
...
## Subagents load CLAUDE.md — `omitClaudeMd` and the rules card

**The off switch is agent frontmatter `omitClaudeMd: true`** (Claude Code CLI 2.1.281). The agent then runs without the user, project and local `CLAUDE.md` *and* without the memory index; managed policy files are kept. The danger is silent: an agent that loses `CLAUDE.md` loses every rule in it, and nothing reports the loss. So the switch comes with a card.

1. **Never set `omitClaudeMd: true` without a card.** Wrap the always-binding rules of the project `CLAUDE.md` in `<!-- card:t0 -->` … `<!-- /card:t0 -->` (markers only, no content change), run `node .claude/scripts/ck/subagent-card.cjs --write`, and make this the first line of the agent body: *"You run without `CLAUDE.md` — before any other step, Read `.claude/cards/subagent-card.md`."*
2. **The card is generated, never hand-edited.** `CLAUDE.md` stays the single source of truth. `/ck:health` fails on a stale card. To regenerate automatically, add `node .claude/scripts/ck/subagent-card.cjs --check >/dev/null 2>&1 || node .claude/scripts/ck/subagent-card.cjs --write || true` as a `SessionStart` hook.
3. **A rule can leave the card only if a gate enforces it.** A rule that a hook or build gate blocks costs nothing to drop. A rule that exists only as text stays in the card.
4. **Pilot on lookup, research and docs agents first**, and measure before extending to implementers and reviewers. The mechanism is proven to load correctly, but its token and quality effect is not yet measured. No kit agent ships with it set.
5. **Route single lookups to `Explore`** rather than switching an implementer off `CLAUDE.md`: it already skips both files.
