...
5. **Run the project's declared handoff steps, in declaration order** — the optional `## Delivery tail` block in the project's CLAUDE.md (see git skill "Delivery Tail" semantics). Execute via `node scripts/ck/delivery-tail.js` — a deterministic executor (no LLM: declared steps carry `run` + `done-when`), the single code path, re-runnable standalone after an interruption. `--dry-run` first on an unfamiliar tail: it resolves placeholders and prints the exact commands without executing. **ClauKit declares no steps of its own: absent/empty block ⇒ no tail, no notice.** `--no-handoff` skips this step.
