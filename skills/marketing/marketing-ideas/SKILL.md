...
Imported from `coreyhaines31/marketingskills` (MIT, (c) 2025 Corey Haines) and adapted for KitForge: the 139-idea catalogue and the guerrilla deep dive are preserved in `references/`; the upstream `.agents/product-marketing.md` context convention is replaced by `plans/marketing-context.md`; the eval harness and the upstream `marketing-plan` / `free-tools` / `referrals` cross-references are dropped (ClauKit does not ship those skills — `/mk:plan`, [[content-strategy]], and [[cro]] cover that ground); category-to-skill execution mapping added.
